...
sonar = pd.read_csv("2026-04-22-15-58.csv")
...
plt.plot(profile_fl["fwd_bin"], profile_fl["median_depth"], lw=2, color="darkorange", label="Forward-looking-90")
plt.xlabel("Projected sonar detection position along forward motion (m)")
plt.ylabel("Altitude relative to sonar (m)")
plt.title("Side-view median profile comparison: Across-track vs Forward-looking-90")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
