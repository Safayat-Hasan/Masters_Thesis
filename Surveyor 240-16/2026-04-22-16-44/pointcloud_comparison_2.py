...
at = pd.read_csv("Reconstructed_across_track_pointcloud.csv")
at = at[["map_y_m", "map_x_m", "altitude_like_sonarview_m"]].dropna().copy()
at.columns = ["x", "y", "z"]
...
axes[0].set_title("Across-track reconstructed")
...
ax.set_title(f"Depth error map (FL reconstructed − AT reconstructed) RMSE={rmse:.3f}m   Bias={bias:+.3f}m")
ax.axis("equal")
...
axes[0].set_xlabel("Across-track reconstructed depth (m)")
axes[0].set_ylabel("Forward-looking reconstructed depth (m)")
...
axes[1].set_xlabel("Depth error: FL reconstructed − AT reconstructed (m)")
