...
class CeruleanYZPointCloudNode(Node):
    def __init__(self) -> None:
        super().__init__("cerulean_yz_pointcloud_node")
        ...
        self.declare_parameter("buffer_pings", 80)

        self.declare_parameter("save_csv", False)
        self.declare_parameter("csv_path", "/tmp/cerulean_live_yz_points.csv")
    ...
    def decode_yz_points(self, data) -> List[Tuple[float, float]]:
        ...
        try:
            if len(yz) > 0 and isinstance(yz[0], (tuple, list)) and len(yz[0]) >= 2:
                return [(float(p[0]), float(p[1])) for p in yz]
        except Exception:
            pass

        try:
            if isinstance(yz, (tuple, list)) and len(yz) % 2 == 0:
                pts = []
        ...
        raise RuntimeError(f"Unsupported YZ format: type={type(yz)} value={yz}")
    ...
    def handle_yz(self, data) -> None:
        ...
        for y_sonar, z_sonar in yz_points:
            try:
                # Rightward fan mapping:
                # x = forward/range direction (to the right)
                # z = spread up/down
                # y = 0 to keep it as a 2D slice in one plane
                x = z_sonar
                y = 0.0
                z = -y_sonar
                ...
                self.get_logger().warn(f"Skipping bad YZ point: {e}")

        if not latest_points:
            self.get_logger().warn(
                "Received YZ packet but no valid points.",
...
def main(args=None) -> None:
    ...
    node: Optional[CeruleanYZPointCloudNode] = None

    try:
        node = CeruleanYZPointCloudNode()
        rclpy.spin(node)
